...
const WhatWeDoMobilePage: React.FC<MobileSubPageProps> = ({ data }) => {
                    ...
                    <div className="mt-6 p-4 text-black">
                        <h1 className="text-3xl font-bold">
                            {data.title.split(' ').map((word, index) => (
                                <span key={index} className={colorLastWords(data.title, 2, index, '#153f57')}>
                                    {word}{' '}
                                </span>
                            ))}
                        </h1>
...
export default WhatWeDoMobilePage;